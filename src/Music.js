...
const zzfxP = (...t) => { let e = zzfxX.createBufferSource(), f = zzfxX.createBuffer(t.length, t[0].length, zzfxR); t.map((d, i) => f.getChannelData(i).set(d)), e.buffer = f, e.connect(zzfxX.destination); return e }
...
export function playMusic() {
    const mySong = zzfxM(...song);
    const node = zzfxP(...mySong);
    node.loop = true;
    node.start();
    return node;
}